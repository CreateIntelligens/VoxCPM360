...
class VoxCPMModel(nn.Module):
    ...
    def optimize(self, disable: bool = False):
        ...
        try:
            ...
            self.base_lm.forward_step = torch.compile(self.base_lm.forward_step, mode="default", fullgraph=True)
            self.residual_lm.forward_step = torch.compile(
                self.residual_lm.forward_step, mode="default", fullgraph=True
            )
            self._feat_encoder_raw = self.feat_encoder
            self.feat_encoder = torch.compile(self.feat_encoder, mode="default", fullgraph=True)
            self.feat_decoder.estimator = torch.compile(
                self.feat_decoder.estimator, mode="default", fullgraph=True
            )
